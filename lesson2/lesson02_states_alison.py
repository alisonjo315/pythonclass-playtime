...
state_abbrs = """AL,AK,AZ,AR,CA,CO,CT,DE,DC,FL,GA,HI,ID,IL,IN,IA,KS,KY,LA,ME,MD,MA,MI,MN,MS,MO,MT,NE,NV,NH,NJ,NM,NY,NC,ND,OH,OK,OR,PA,RI,SC,SD,TN,TX,UT,VT,VA,WA,WV,WI,WY"""

state_abbrs_list = state_abbrs.split(',')
#print state_abbrs_list
state_names_list = state_names.split(',')

output = """<select>"""

for abbr,name in zip(state_abbrs_list, state_names_list):
    output += """
        <option value="{0}">{1}</option>""".format(abbr, name)
